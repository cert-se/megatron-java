...
# Executes Megatron in dev.
# All paths are relative to the installation directory.

#export SITIC_JAVA=/usr/local/jre-1.5.0/bin/java
...
echo `date`: Megatron Starts.
$SITIC_JAVA $SITIC_JAVA_OPTIONS $SITIC_JCONSOLE_OPTIONS -cp $SITIC_CLASSPATH -Dmegatron.configfile=conf/dev/megatron-globals.properties Megatron $*
