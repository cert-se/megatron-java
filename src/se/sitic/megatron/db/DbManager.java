...
public class DbManager {
	...
	public Organization searchOrgForDomain(String domainName)
		...
		try {
			...
			throw handleException(e.getClass().getSimpleName() + " exception in searchOrgByDomain", e);
		}
...
*		 Fetches log entries for specified organization for use in OrganizationReportGenerator.
	...
	public List<LogEntry> fetchLogEntriesForOrganizationReport(int orgId, Date startDate, Date endDate, List<String> jobTypes) throws DbException {
		...
		if (jobTypes.isEmpty() == false) {
			String jobTypeNames = "";
			int index;
			for (index = 0; index < jobTypes.size(); index++) {
				jobTypeNames = jobTypeNames +  "'" + jobTypes.get(index) + "'";

				if (index < jobTypes.size() - 1) {
					jobTypeNames = jobTypeNames + ", ";
				}
			}
			String sql = "SELECT id from log_entry le, job j WHERE j.finished >= ? AND j.finished <= ? AND j.job_type_id in (" + jobTypes + ") AND le.job_id = j.id AND le.org_id = ?;";

			Query query = session.createQuery(sql);

			query.setLong(0, SqlUtil.convertTimestamp(startDate));
			query.setLong(1, SqlUtil.convertTimestamp(endDate));
			query.setInteger(2, orgId);

			Iterator<?> iter = query.list().iterator();			
			while(iter.hasNext()) {
				long entryID = ((Long) iter.next()).longValue();
	...
	public long getNumberOfMatchedLogEntries(Date startDate, Date endDate) 
		...
		try {
			...
			/** No. of log entries that have been matched to an organization. */
	...
	public List<BigInteger> isRangeOverlappingOrgId(long start, long end) 
		...
		try {
			...
			throw handleException(e.getClass().getSimpleName() + " exception in isRangeOverlappingOrgId", e);
		}           
	}
